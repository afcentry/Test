...
from BotPlugin.NmapScaner import NmapScaner
...
def CTYBot_ExecPlugin_Each_CallBack_RunParam(iPluginID, strDestHost):
    ...
    elif (iPluginID == CSkLB_Config_RemoteClient_BaseDef.s_iExec_NmapPortService):  # 获取IP对应的端口服务情况
        strRunResult = CTYLB_Sk_Plugin_BeeExecCmd.Bee_Exec_NmapPortService_EachHost(strDestHost)
        iReplyPluginID = CSkLB_Config_RemoteClient_BaseDef.s_iExec_NmapPortService_Reply
...
def CTYBot_ExecPlugin_GetSupportIDArray():
    retArray = [
        ...
        CSkLB_Config_RemoteClient_BaseDef.s_iExec_NmapPortService,
    ]
    return retArray
