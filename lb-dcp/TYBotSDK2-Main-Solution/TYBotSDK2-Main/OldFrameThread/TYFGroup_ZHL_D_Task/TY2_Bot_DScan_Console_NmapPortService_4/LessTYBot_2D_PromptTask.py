...
from Tylb_Bot_Sock_Prompt_D_Task import CTYBot_Sock_D_PromptTask, CTYBot_WebName_DTask2_Base, \
    CTYFBot_DTask_OPDB_Prompter
...
s_iExec_NmapPortService = 505  # nmap扫描端口服务
s_iExec_NmapPortService_Reply = 506  # nmap扫描端口服务，回复

g_bSysRunning = True  # 系统运行标识


class CTYFBot_D_OPDB_GetNmapPortService(CTYFBot_DTask_OPDB_Prompter):
    def __init__(self, hlSockMang, iEachBlockCount=50):
        strTaskAppName = 'GetNmapPortService'
        strSubTaskName = datetime.now().strftime('%Y_%m_%d#%H_%M_%S')
        strDBName = 'ty2_zhl_enterprises'
        strTableName = 'ty2_domain_ip'
        strFieldName = 'ip'
        strParamSignIDField = 'id'
        CTYFBot_DTask_OPDB_Prompter.__init__(self, hlSockMang, strTaskAppName, strSubTaskName, 'GetNmapPortService',
                                             strDBName, strTableName, strFieldName, strParamSignIDField,
                                             s_iExec_NmapPortService, iEachBlockCount)
        pass
...
if __name__ == '__main__':
    ...
    if not os.path.isfile(config_file):
        CTYLB_Log.ShowLog(0, 'Main', '[%s] config file missing...Quit!' % ('DScan_Console'))
        msgObject = {
            "monitor_type": "status",
            "level": "error",
            "target": "",
    ...
    # 增加参数, nmap扫描端口服务
    getNmapPortServiceTask = CTYFBot_D_OPDB_GetNmapPortService(g_execTYLB_SockBot_Req_DTask_Frame.s_hlSockMang)
    g_execTYLB_SockBot_Req_DTask_Frame.PromptDTask(getNmapPortServiceTask)
    ...
    while (g_bSysRunning):
        ...
        if (not bTaskBusy):
            time.sleep(0.1)
    # 退出
    getNmapPortServiceTask.Vilu_CloseQuit()
